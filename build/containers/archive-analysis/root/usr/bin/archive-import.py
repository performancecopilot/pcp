...
def update_dashboard_time_window(begin: str, end: str, json_path: str):
    ...
    logging.info("Dashboard time window updated successfully.")


def setup_grafana(path: Path, i: int, count: int, dashboard_path: str):
    ...
    update_dashboard_time_window(minimum_start_time, maximum_finish_time, dashboard_path)
...
def poll(archives_path: str, dashboard_path: str, no_op: bool):
    ...
    if not archive_paths:
        ...
        for i, path in enumerate(archive_paths, start=1):
            setup_grafana(path, i, len(archive_paths), dashboard_path)
        for i, path in enumerate(archive_paths, start=1):
