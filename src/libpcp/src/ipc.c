...
version_locked(int fd)
{
	...
	if (pmDebugOptions.context)
	    fprintf(stderr,
		"IPC protocol botch: version: table->" PRINTF_P_PFX "%p fd=%d sz=%zu\n",
...
socket_locked(int fd)
{
	...
	if (pmDebugOptions.context)
	    fprintf(stderr,
		"IPC protocol botch: socket: table->" PRINTF_P_PFX "%p fd=%d sz=%zu\n",
		__pmIPCTable, fd, ipctablecount);
	return 0;
    }
...
features_locked(int fd)
{
	...
	if (pmDebugOptions.context)
	    fprintf(stderr,
		"IPC protocol botch: features: table->" PRINTF_P_PFX "%p fd=%d sz=%zu\n",
		__pmIPCTable, fd, ipctablecount);
	return 0;
    }
