...
class PCP2OPENTELEMETRY(object):
    ...
    def write_opentelemetry(self, timestamp):
        ...
        if not results:
            sys.stderr.write("ERROR: metric(s) invalid\n")
            sys.exit(1)
        ...
        # produce otel UCUM units
        def unit_function(units):
            # Space unit prefixes (powers of 1024 for "binary" units)
            def UCUM_space_prefix(scale):
                if scale == 0:
                    return "" # base bytes
                elif scale == 1:
                    return "Ki" # Kibibyte
