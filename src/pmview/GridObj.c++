...
GridObj::GridObj(OMC_Bool onFlag, 
...
void
GridObj::setTran(float xTran, float zTran, uint_t setWidth, uint_t setDepth)
{
    uint_t		i, j;
    uint_t		totalWidth;
    uint_t		totalDepth;
    ...
    OMC_IntVector	rowPos(rows());
    OMC_IntVector	colPos(cols());

    assert(_finished == OMC_true);
...
int
addRowCols (OMC_IntVector & ivec, int nsize, int min)
{
    int extra = 0;

    if (nsize > ivec.length()) {
	extra = min * (nsize - ivec.length());

	ivec.resizeCopy(nsize, min);
    }
...
void
GridObj::addObj(ViewObj *obj, uint_t col, uint_t row)
{
    uint_t	i;
    GridItem	newItem;
...
GridObj::finishedAdd()
{
	...
	if (item._item->cols() > 1) {
	    ...
	    if (current < size) {
		size -= current;
		adjust = (uint_t)((size / (float)item._item->cols()) + 0.5);
		for (j = item._col; 
	...
	if (item._item->rows() > 1) {
	    ...
	    if (current < size) {
		size -= current;
		adjust = (uint_t)((size / (float)item._item->rows()) + 0.5);
		for (j = item._row; 
...
ostream&
operator<<(ostream& os, GridObj const& rhs)
{
    rhs.display(os);
    return os;
}
...
void
GridObj::display(ostream& os) const
{
    uint_t	i;
    uint_t	sum;
    ...
    os << ", minRowDepth = " << _minDepth << ", minColWidth = "
       ...
       << ", finishedAdd = " << (_finished == OMC_true ? "true" : "false")
       << endl;

    os << "Column widths: ";
    for (i = 0, sum = 0; i < _colWidth.length(); i++) {
	os << _colWidth[i] << ' ';
	sum += _colWidth[i];
    }
    os << endl;
    ...
    os << "Row depths: ";
    for (i = 0, sum = 0; i < _rowDepth.length(); i++) {
	os << _rowDepth[i] << ' ';
	sum += _rowDepth[i];
    }
    os << endl;
