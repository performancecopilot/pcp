...
: ModObj(baseFlag, defaults, x, y, cols, rows, align),
  ...
  _bars(0),
  _metDir(towards),
  _metLabels(new OMC_StrList),
  _instDir(away),
  _instLabels(new OMC_StrList)
{
    _objtype |= BAROBJ;
...
BarObj::finishedAdd()
{
    ...
    uint_t		i;
    uint_t		max = 0;
    uint_t		numMetLabels = 0;
    uint_t		numInstLabels = 0;
    ...
    if (_metLabels->length() || _instLabels->length()) {
    ...
    if (_colors.length())
	colSpec = theColorLists.list(_colors.ptr());
    ...
    if (colSpec != NULL) {
		...
		if (colSpec->_list.length() == 0)
		    colScale = new INV_ColorScale(0.0, 0.0, 1.0);
		else {
		    colScale = new INV_ColorScale(*(colSpec->_list[0]));
		    for (i = 1; i < colSpec->_list.length(); i++)
			colScale->add(new INV_ColorStep(*(colSpec->_list[i]),
							colSpec->_max[i]));
		}
	    }
	}
	...
	_metrics.resolveColors(INV_MetricList::perMetric);
    }
	...
	if (_metLabels->length() < _metrics.numMetrics())
	    numMetLabels = _metLabels->length();
	else
	...
	if (_instLabels->length() < max)
	    numInstLabels = _instLabels->length();
	else
	    numInstLabels = max;
...
void
BarObj::setTran(float xTran, float zTran, uint_t setWidth, uint_t setDepth)
{
...
ostream&
operator<<(ostream& os, BarObj const& rhs)
{
    rhs.display(os);
    return os;
}
...
void
BarObj::display(ostream& os) const
{
    BaseObj::display(os);
...
Text **
BarObj::calcLabels(const OMC_StrList &labels, LabelSide side, uint_t numLabels)
{
    Text		**text = NULL;
    uint_t		i;
    uint_t		maxWidth = 0;
...
SoNode *
BarObj::doLabels(Text **text, LabelSide side, uint_t numLabels)
{
