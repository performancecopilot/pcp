...
Text::Text(const OMC_String &theString, 
	   Direction theDir, 
  ...
  _translation(0)
{
	...
	theMediumFont->ref();
	theLargeFont->ref();
	theBoxAction = new SoGetBoundingBoxAction(theViewer->getViewportRegion());
    }
	...
	theText->string.setValue(theString.ptr());
	...
	char *hasLow = strpbrk(theString.ptr(), theHeightStr);
...
ostream&
operator<<(ostream& os, Text const& rhs)
{
    rhs.display(os);
    return os;
}
