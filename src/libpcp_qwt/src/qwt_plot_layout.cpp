...
namespace
{
    class LayoutEngine
    {
      public:
        ...
        inline void setFixedAxisOffset( int axisPos, int offset ) { m_fixedAxisOffset[ axisPos ] = offset; }
        inline int fixedAxisOffset( int axisPos ) const { return m_fixedAxisOffset[ axisPos ]; }
      ...
      private:
        ...
        unsigned int m_fixedAxisOffset[QwtAxis::AxisPositions];
        ...
        {
            {
                ...
                if ( scaleData.isVisible )
                {
                    ...
                    if (m_fixedAxisOffset[axisPos] > 0)
                    {
                        dimensions.setDimAxis( axisId,
                            m_fixedAxisOffset[axisPos] + backboneOffset[axisPos]);
                        continue;
                    }
...
/*!
   Set a fixed axis offset for the given axis scale.
   The offset is the space between an outer edge of the plot widget
   and the scale backbone.
 
   \param offset New offset
   \param axis One of QwtPlot::Axis.
   Specifies which axis to make fixed offset, -1 means margin at all borders.
   \sa fixedAxisOffset() 
 */
void QwtPlotLayout::setFixedAxisOffset( int offset, int axisPos )
{
    if ( offset < 0 )
        offset = 0;
 ...
 */
int QwtPlotLayout::fixedAxisOffset( int axisPos ) const
{
    if ( !QwtAxis::isValid( axisPos ) )
        return 0;

    return m_data->engine.fixedAxisOffset( axisPos );
}
