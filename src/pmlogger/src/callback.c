...
do_work(task_t *tp)
{
	...
	peek_offset = archctl.ac_tell_cb(&archctl, 0, caller);
	peek_offset += pdu_payload - sizeof(__pmPDUHdr) + 2*sizeof(int);
	if (peek_offset > max_offset) {
	    if (pmDebugOptions.appl2)
		pmNotifyErr(LOG_INFO, "callback: new volume based on max size, currently %ld", archctl.ac_tell_cb(&archctl, 0, caller));
	...
	if (archctl.ac_tell_cb(&archctl, 0, caller) > flushsize) {
	    needti = 1;
	    if (pmDebugOptions.appl2)
		pmNotifyErr(LOG_INFO, "callback: file size (%ld) reached flushsize (%ld)", archctl.ac_tell_cb(&archctl, 0, caller), (long)flushsize);
	}
    ...
    if (exit_bytes != -1 && 
        (vol_bytes + archctl.ac_tell_cb(&archctl, 0, caller) >= exit_bytes))
    ...
    if (vol_switch_bytes > 0 &&
        (archctl.ac_tell_cb(&archctl, 0, caller) >= vol_switch_bytes)) {
        (void)newvolume(VOL_SW_BYTES);
	if (pmDebugOptions.appl2)
	    pmNotifyErr(LOG_INFO, "callback: new volume based on size (%ld)", archctl.ac_tell_cb(&archctl, 0, caller));
    }
}
