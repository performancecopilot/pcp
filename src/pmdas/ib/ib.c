...
ib_fetch_val(pmdaMetric *mdesc, unsigned int inst, pmAtomValue *atom)
{
	...
	if ((st = pmdaCacheLookup (mdesc->m_desc.indom, inst, &name,
		...
		st = PM_ERR_INST;
	    __pmNotifyErr (LOG_ERR, "Cannot find instance %d in indom %s\n",
			   pmInDomStr(mdesc->m_desc.indom), pmErrStr(st));
	    return (st);
	}
    }
...
/* A graby hack to intercept calls to ibpanic - ibpanic in
 * libibcommon calls exit which is not something one wants
 * to do in the DSO context */
void
