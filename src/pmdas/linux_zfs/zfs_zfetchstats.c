	...
	if (fp != NULL) {
		while (getline(&line, &len, fp) != -1) {
                        if (regexec(rgx_row, line, nmatch, pmatch, 0) == 0) {
                                ...
                                mname[len_mn] = '\0';
                                mval[len_mv] = '\0';
				if (strcmp(mname, "hits")) zfetchstats->hits = atoi(mval);
				else if (strcmp(mname, "misses")) zfetchstats->misses = atoi(mval);
				else if (strcmp(mname, "max_streams")) zfetchstats->max_streams = atoi(mval);
                        }
                        free(mname);
                        free(mval);
                }
