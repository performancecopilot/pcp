...
when($user_agent)->get('/api/jolokia/read/java.lang:type=Memory')->then_return({'value' => {'NonHeapMemoryUsage' => { 'max' => 1234 } }});
is($jvm_memory->attribute_for('non_heap_memory_usage', 'max'), 1234, "non_heap_memory_usage attribute is available");

when($user_agent)->get('/api/jolokia/read/java.lang:type=Memory')->then_return({'value' => {'HeapMemoryUsage' => { 'max' => 1234 } }});
