...
static proc_pid_list_t procpids;

//
//
// NOTE:   threads should be pulled out of pids !!!!
//
//


//
// All the hotproc goodness
//
// PIDS that we are keeping track of as "hot" candidates
// need a seperate list since it is generated by the timer update
// might not need this if we just use hotproc_list
...
// possible procs as candidates for hot
...
check_if_hot( char *cpid ){
	...
	// Real logic is:
	// Lookup pid in hash that has stats computed from timer below
	// if metrics match config, then return true
	// also, this should catch procs that wre not started when
	// we did the stats computation, sice they just wont be there
	// also don;t have to worry about procs that no longer exist but
	// that are in stats list, since they are never checked (right?)
...
init_hotproc_list(void)
{
    ...
    hotproc_list[0] = (process_t*)malloc(INIT_PROC_MAX * sizeof(process_t));
    hotproc_list[1] = (process_t*)malloc(INIT_PROC_MAX * sizeof(process_t));
    if (hotproc_list[0] == NULL || hotproc_list[1] == NULL || active_list == NULL)
        return -oserror();
    return 0;
}
...
add_active_list(process_t *node, config_vars *vars)
{
    ...
    // Dummy for now.  Just check if burn is > ~0
    //if( vars->cpuburn < 0.01 ){
	//return 0;
    //}	
    //else{
	//fprintf(stderr, "Added hotproc %d\n", node->pid);
...
if ((afid = __pmAFregister(&interval, NULL, hotproc_timer)) < 0) {
...
lookup_node(int curr_prev, pid_t pid)
{
    ...
    if ( (numprocs[curr_prev] > 0) &&
         ((node = bsearch(&key, hotproc_list[curr_prev], numprocs[curr_prev],
...
int sts;
    ...
    // Get list of pids in hotpids

//  Probably call the proper refresh func instead here

    hotpids.count = 0;
    hotpids.threads = 0;  //Fix this
    refresh_global_pidlist( 0, &hotpids );
    // Init hash with ep if new pid, cull old pids
    refresh_proc_pidlist( &hotproc_poss_pid, &hotpids );

    fprintf(stderr,"Got Proc list with %d procs!\n", hotpids.count);

    for (i=0; i < hotpids.count; i++) {

	pid = hotpids.pids[i];

	//fprintf(stderr,"Working on Proc %d!\n", i);

	// Must (should?) be there due to 2 calls above
        node = __pmHashSearch(hotpids.pids[i], &hotproc_poss_pid.pidhash);

	if( node == NULL){
		fprintf(stderr,"Hash search failed for Proc %d!\n", i);
		continue;
	}
	...
	//fprintf(stderr,"Got statentries\n");
	...
	//fprintf(stderr,"lookupnode yes\n");
		...
		//fprintf(stderr, "%d : cpuburn %f\n", pid, vars.cpuburn);


		}
        	else {
	//fprintf(stderr,"lookupnode no\n");
            		newnode->r_cpuburn = 0;
...
//fprintf(stderr,"rss\n");
    ...
    qsort(hotproc_list[current], numprocs[current],
...
refresh_proc_pid(proc_pid_t *proc_pid, int threads, const char *cgroups)
{
    ...
    sts = (cgroups && cgroups[0] != '\0') ?
		...
		refresh_global_pidlist(threads, &procpids);
    if (sts < 0)
	return sts;
