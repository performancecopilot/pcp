...
# mpss 3.3 rpm puts the python files in /usr/src
...
# Percore
				("cpu_usr", POINTER(c_ulonglong)),
				("cpu_sys", POINTER(c_ulonglong)),
				("cpu_nice", POINTER(c_ulonglong)),
				("cpu_idle", POINTER(c_ulonglong))]

	def __init__(self, cores):
		self.num_cores = cores

		pfu = (c_ulonglong * cores)()
		self.cpu_usr = cast(pfu, POINTER(c_ulonglong))

		pfs = (c_ulonglong * cores)()
		self.cpu_sys = cast(pfs, POINTER(c_ulonglong))

		pfn = (c_ulonglong * cores)()
		self.cpu_nice = cast(pfn, POINTER(c_ulonglong))

		pfi = (c_ulonglong * cores)()
		self.cpu_idle = cast(pfi, POINTER(c_ulonglong))
...
class MicPMDA(PMDA):
	...
	# Card indom. (Cores will be iterated by metrics)
	numcards = 0
	cards = {}
	...
	# Keep track of max number of cores found for metric count. Could have heterogeneous cards
	maxcores = 0
	...
	def init_mic(self):
		...
		for device in range(self.numcards):
			...
			numcores = mic.mic_get_cores_count()
			if( numcores > self.maxcores ):
				self.maxcores = numcores
	...
	def update_mic(self):
		...
		for device in range(self.numcards):
			...
			# Overall metrics
			devinfo.all_usr = 1000 * mic.mic_get_user_sum() / (devinfo.num_threads * self.HZ)
			devinfo.all_sys = 1000 * mic.mic_get_sys_sum() / (devinfo.num_threads * self.HZ)
			devinfo.all_nice = 1000 * mic.mic_get_nice_sum() / (devinfo.num_threads * self.HZ)
			devinfo.all_idle = 1000 * mic.mic_get_idle_sum() / (devinfo.num_threads * self.HZ)

			# Per core metrics
			core_usr = mic.mic_get_user_counters()
			cnt = 0
			for usr in core_usr:
				devinfo.cpu_usr[cnt] = 1000 * usr /  (devinfo.num_threads * self.HZ)
				cnt += 1

			core_sys = mic.mic_get_sys_counters()
			cnt = 0
			for sys in core_sys:
				devinfo.cpu_sys[cnt] = 1000 * sys / (devinfo.num_threads * self.HZ)
				cnt += 1

			core_nice = mic.mic_get_nice_counters()
			cnt = 0
			for nice in core_nice:
				devinfo.cpu_nice[cnt] = 1000 * nice / (devinfo.num_threads * self.HZ)
				cnt += 1

			core_idle = mic.mic_get_idle_counters()
			cnt = 0
			for idle in core_idle:
				devinfo.cpu_idle[cnt] = idle
				cnt += 1
	...
	def mic_fetch_percore_callback(self, item, inst):
		...
		if( core < micdata.num_cores):
			if( counter == 0 ):
				return [micdata.cpu_usr[core], 1]
			elif( counter == 1 ):
				return [micdata.cpu_sys[core], 1]
			elif( counter == 2 ):
				return [micdata.cpu_nice[core], 1]
			else:
				return [micdata.cpu_idle[core], 1]
		else:
			return [c_api.PM_ERR_PMID, 0]
	...
	def __init__(self, name, domain):
		...
		# Percore
		for cores in range(self.maxcores):
			self.add_metric(name + ".mgmt.cores.core%d.usr"%cores, pmdaMetric(self.pmid(self.PERCORE_CLUSTER, cores*4),
				c_api.PM_TYPE_U64,  self.card_indom, c_api.PM_SEM_COUNTER,
				pmUnits(0, 1, 0, 0, c_api.PM_TIME_MSEC, 0)),
				"User time per core")

			self.add_metric(name + ".mgmt.cores.core%d.sys"%cores, pmdaMetric(self.pmid(self.PERCORE_CLUSTER, cores*4 + 1),
				c_api.PM_TYPE_U64,  self.card_indom, c_api.PM_SEM_COUNTER,
				pmUnits(0, 1, 0, 0, c_api.PM_TIME_MSEC, 0)),
				"System time per core")
		
			self.add_metric(name + ".mgmt.cores.core%d.nice"%cores, pmdaMetric(self.pmid(self.PERCORE_CLUSTER, cores*4 + 2),
				c_api.PM_TYPE_U64,  self.card_indom, c_api.PM_SEM_COUNTER,
				pmUnits(0, 1, 0, 0, c_api.PM_TIME_MSEC, 0)),
				"Nice time per core")
		
			self.add_metric(name + ".mgmt.cores.core%d.idle"%cores, pmdaMetric(self.pmid(self.PERCORE_CLUSTER, cores*4 + 3),
				c_api.PM_TYPE_U64,  self.card_indom, c_api.PM_SEM_COUNTER,
				pmUnits(0, 1, 0, 0, c_api.PM_TIME_MSEC, 0)),
				"Idle time per core")
