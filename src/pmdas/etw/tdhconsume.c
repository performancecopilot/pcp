...
void
enableKernelTrace(ULONG enableFlags, BOOL useGlobalSequence)
{
...
retrySession:
    ...
    kernelProperties->EnableFlags = enableFlags;
...
main(int argc, LPTSTR *argv)
{
    ...
    if (sysFlags) {
	enableKernelTrace(sysFlags, useGlobalSequence);
