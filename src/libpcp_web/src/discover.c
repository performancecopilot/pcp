...
static void
discover_creation_timestamp(pmDiscover *p, __pmTimestamp *stamp)
{
...
pmDiscoverNewSource(pmDiscover *p, int context)
{
    ...
    discover_creation_timestamp(p, &stamp);
 ...
 * Return value indicates any remaining unprocessed input
 * (additional streamed data required to complete this).
 */
static ssize_t
...
pmDiscoverStreamMeta(pmDiscover *p, const char *content, size_t length)
{
	...
	int	type = p->context.type | PM_CTXFLAG_LAST_VOLUME;
	...
	if (hdr.len <= sizeof(__pmLogHdr)) {
	    infofmt(msg, "Unknown metadata record type %d (0x%02x), len=%d\n",
			hdr.type, hdr.type, hdr.len);
	    moduleinfo(p->module, PMLOG_ERROR, msg, p->data);
	    return -EINVAL;
	}
	len = hdr.len - sizeof(__pmLogHdr); /* includes trailer */
	if (bytes < len) {
	    mmv_inc(data->map, data->metrics[DISCOVER_META_PARTIAL_READS]);
	    break;     /* insufficient data to continue */
	}

	buf += sizeof(__pmLogHdr);
	if (pmDebugOptions.discovery)
	    fprintf(stderr, "Streaming metadata record %zu bytes, type %s\n",
			len, __pmLogMetaTypeStr(hdr.type));

	if ((sts = stream_metadata(p, hdr.type, (uint32_t *)buf, len)) < 0) {
...
pmDiscoverStreamData(pmDiscover *p, const char *content, size_t length)
{
	...
	if (pmDebugOptions.discovery)
	    fprintf(stderr, "Streaming data sample %u bytes\n", buflen);

	if ((pb = __pmFindPDUBuf(buflen)) == NULL) {
	    sts = -oserror();
	    infofmt(msg, "Failed to find %u byte buffer: %s\n",
			    buflen, pmErrStr(sts));
	    moduleinfo(p->module, PMLOG_ERROR, msg, p->data);
	    return sts;
	}
	...
	bytes -= sizeof(uint32_t) + buflen;
	buf += sizeof(uint32_t) + buflen;
    }

    if (buf - p->datavol == bytes) {
	/* buffer was completely processed, clear for later reuse */
	sdsclear(p->datavol);
    } else if (p->datavol != buf) {
	/* keep just the remaining part for subsequent processing */
	msg = p->datavol;
	p->datavol = sdsnewlen(buf, sdslen(p->datavol) - bytes);
	sdsfree(msg);
    }
