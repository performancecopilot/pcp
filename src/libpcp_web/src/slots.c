...
#else
static int keysClusterLibuvAttach() { return RESP_OK; }
#endif
...
key_server_disconnect_callback(const keysAsyncContext *keys, int status)
{
	...
	if (keys->c.connection_type == REDIS_CONN_UNIX)
...
keySlotsInit(dict *config, void *events)
{
    ...
    sts = keysClusterLibuvAttach(slots->acc, slots->events);
    if (sts != REDIS_OK) {
		...
		void *userdata, void *events, void *arg)
{
    ...
    keySchemaLoad(slots, flags, info, done, userdata, events, arg);
}
		...
		keyClusterCallbackFn *callback, void *arg)
{
    ...
    if ((sts = keyClusterAsyncFormattedCommand(slots->acc,
		    keySlotsReplyCallback, srd, cmd, size)) != REDIS_OK) {
	mmv_inc(slots->map, slots->metrics[SLOT_REQUESTS_ERROR]);
	...
	keyClusterCallbackFn *callback, void *arg)
{
    ...
    if (respReaderFeed(reader, buffer, nread) != REDIS_OK) {
