...
int pmIniFileParse(sds tool, ini_handler handler, void* user)
{
    ...
    ini_parse(sysconf_dir_config, handler, user);
    ini_parse(homedir_pcp_config, handler, user);
    ini_parse(homedir_config, handler, user);
    return ini_parse(cwd_config, handler, user);
}
