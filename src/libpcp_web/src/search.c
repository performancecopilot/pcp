		...
		const char *oneline, const char *helptext, void *arg)
{
	...
	length += 2;
    cmd = key_command(length);

    cmd = key_param_str(cmd, FT_ADD, FT_ADD_LEN);
    cmd = key_param_str(cmd, FT_TEXT_KEY, FT_TEXT_KEY_LEN);
    docid = keys_search_docid(FT_TEXT_KEY, typestr, name);
    cmd = key_param_sds(cmd, docid);
    sdsfree(docid);
    cmd = key_param_str(cmd, "1", 1);

    cmd = key_param_str(cmd, FT_REPLACE, FT_REPLACE_LEN);
    cmd = key_param_str(cmd, FT_PARTIAL, FT_PARTIAL_LEN);

    length = pmsprintf(buffer, sizeof(buffer), "%u", type);
    cmd = key_param_str(cmd, FT_PAYLOAD, FT_PAYLOAD_LEN);
    cmd = key_param_str(cmd, buffer, length);

    cmd = key_param_str(cmd, FT_FIELDS, FT_FIELDS_LEN);
    cmd = key_param_str(cmd, FT_NAME, FT_NAME_LEN);
    cmd = key_param_str(cmd, name, strlen(name));
    cmd = key_param_str(cmd, FT_TYPE, FT_TYPE_LEN);
    cmd = key_param_str(cmd, typestr, strlen(typestr));
    if (indom && *indom != '\0') {
	cmd = key_param_str(cmd, FT_INDOM, FT_INDOM_LEN);
	cmd = key_param_str(cmd, indom, strlen(indom));
    }
    if (oneline && *oneline != '\0') {
	cmd = key_param_str(cmd, FT_ONELINE, FT_ONELINE_LEN);
	cmd = key_param_str(cmd, oneline, strlen(oneline));
    }
    if (helptext && *helptext != '\0') {
	cmd = key_param_str(cmd, FT_HELPTEXT, FT_HELPTEXT_LEN);
	cmd = key_param_str(cmd, helptext, strlen(helptext));
    }
...
keys_search_info(keySlots *slots, sds pcpkey, void *arg)
{
    ...
    cmd = key_command(2);
    cmd = key_param_str(cmd, FT_INFO, FT_INFO_LEN);
    cmd = key_param_sds(cmd, key);
    sdsfree(key);
...
keys_search_text_query(keySlots *slots, pmSearchTextRequest *request, void *arg)
{
    ...
    cmd = key_command(length);
    cmd = key_param_str(cmd, FT_SEARCH, FT_SEARCH_LEN);
    cmd = key_param_sds(cmd, key);
    ...
    sdsfree(base_query);
    cmd = key_param_sds(cmd, query);
    sdsfree(query);

    cmd = key_param_str(cmd, FT_WITHSCORES, FT_WITHSCORES_LEN);
    cmd = key_param_str(cmd, FT_WITHPAYLOADS, FT_WITHPAYLOADS_LEN);

    if (infields) {
	cmd = key_param_str(cmd, FT_INFIELDS, FT_INFIELDS_LEN);
    ...
    if (returns) {
	cmd = key_param_str(cmd, FT_RETURN, FT_RETURN_LEN);
    ...
    if (highlights) {
	cmd = key_param_str(cmd, FT_HIGHLIGHT, FT_HIGHLIGHT_LEN);
	cmd = key_param_str(cmd, FT_FIELDS, FT_FIELDS_LEN);
    ...
    cmd = key_param_str(cmd, FT_SCORER, FT_SCORER_LEN);
    cmd = key_param_str(cmd, FT_SCORER_BM25, FT_SCORER_BM25_LEN);

    cmd = key_param_str(cmd, FT_LIMIT, FT_LIMIT_LEN);
    length = pmsprintf(buffer, sizeof(buffer), "%u", request->offset);
    cmd = key_param_str(cmd, buffer, length);
    if (request->count == 0) {
	cmd = key_param_sds(cmd, resultcount_str);
...
keys_search_text_indom(keySlots *slots, pmSearchTextRequest *request, void *arg)
{
    ...
    cmd = key_param_str(cmd, FT_SORTBY, FT_SORTBY_LEN);
    cmd = key_param_str(cmd, "2", 1);
    cmd = key_param_str(cmd, FT_TYPE, FT_TYPE_LEN);
    cmd = key_param_str(cmd, FT_ASC, FT_ASC_LEN);
...
keys_load_search_schema(void *arg)
{
     ...
     */
    key = sdsnewlen(FT_TEXT_KEY, FT_TEXT_KEY_LEN);
    cmd = key_command(3 + 3 + 5 + 2 + 4 + 4);

    cmd = key_param_str(cmd, FT_CREATE, FT_CREATE_LEN);
    cmd = key_param_str(cmd, FT_TEXT_KEY, FT_TEXT_KEY_LEN);
    cmd = key_param_str(cmd, FT_SCHEMA, FT_SCHEMA_LEN);

    cmd = key_param_str(cmd, FT_TYPE, FT_TYPE_LEN);
    cmd = key_param_str(cmd, FT_TAG, FT_TAG_LEN);
    cmd = key_param_str(cmd, FT_SORTABLE, FT_SORTABLE_LEN);

    cmd = key_param_str(cmd, FT_NAME, FT_NAME_LEN);
    cmd = key_param_str(cmd, FT_TEXT, FT_TEXT_LEN);
    cmd = key_param_str(cmd, FT_WEIGHT, FT_WEIGHT_LEN);
    cmd = key_param_str(cmd, "9", sizeof("9")-1);
    cmd = key_param_str(cmd, FT_SORTABLE, FT_SORTABLE_LEN);
    ...
    cmd = key_param_str(cmd, FT_TEXT, FT_TEXT_LEN);
    cmd = key_param_str(cmd, FT_WEIGHT, FT_WEIGHT_LEN);
    cmd = key_param_str(cmd, "4", sizeof("4")-1);

    cmd = key_param_str(cmd, FT_HELPTEXT, FT_HELPTEXT_LEN);
    cmd = key_param_str(cmd, FT_TEXT, FT_TEXT_LEN);
    cmd = key_param_str(cmd, FT_WEIGHT, FT_WEIGHT_LEN);
    cmd = key_param_str(cmd, "2", sizeof("2")-1);
