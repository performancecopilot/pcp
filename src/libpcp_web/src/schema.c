...
static void
keys_map_publish_callback(
	keyClusterAsyncContext *c, void *r, void *arg)
{
    keyMapBaton	*baton = (keyMapBaton *)arg;
    respReply          *reply = r;

    seriesBatonCheckMagic(baton, MAGIC_MAPPING, "keys_map_publish_callback");
...
static void
keys_map_publish(keyMapBaton *baton)
{
    char		hash[42];
    sds			msg, key, cmd;
    ...
    keySlotsRequest(baton->slots, cmd,
			keys_map_publish_callback, baton);
    sdsfree(cmd);
}

static void
keys_map_request_callback(
	mapClusterAsyncContext *c, void *r, void *arg)
{
    keyMapBaton	*baton = (keyMapBaton *)arg;
    respReply          *reply = r;
    int			newname;
...
keyMapRequest(keyMapBaton *baton, keyMap *map, sds name, sds value)
{
    ...
    keySlotsRequest(baton->slots, cmd, redis_map_request_callback, baton);
    sdsfree(cmd);
}

void
redisGetMap(keySlots *slots, keyMap *mapping, unsigned char *hash, sds mapStr,
...
keys_series_source(keySlots *slots, void *arg)
{
    ...
    sdsfree(key);
    keySlotsRequest(slots, cmd, redis_context_name_source, arg);
    sdsfree(cmd);
...
static void
initKeyStreamBaton(redisStreamBaton *baton, keySlots *slots,
...
static void
doneKeyStreamBaton(redisStreamBaton *baton)
{
    void		*load = baton->arg;
...
void
keys_series_mark(keySlots *redis, sds timestamp, int data, void *arg)
{
    seriesLoadBaton		*baton = (seriesLoadBaton *)arg;
