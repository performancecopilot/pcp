...
pcp_pmda_exec(void* args) {
    struct agent_config* config = ((struct pcp_args*)args)->config;
    chan_t* pcp_to_aggregator = ((struct pcp_args*)args)->aggregator_request_channel;
    chan_t* aggregator_to_pcp = ((struct pcp_args*)args)->aggregator_response_channel;
    (void)config;
