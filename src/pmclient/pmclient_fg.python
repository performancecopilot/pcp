...
class sampler(object):
    def __init__(self, fg):
        ...
        self.timestamp = fg.extend_timestamp()
    ...
    def get_sample(self):
        ...
        self.peak_cpu = 0
        for (ucode, uname, uvalue), (scode, sname, svalue) in zip(self.cpu_user(), self.cpu_sys()):
            assert (ucode == scode) # indoms should match
            try:
                u = uvalue() + svalue()
                if u > 1.0: # small errors are possible, so clip the utilization at 1.0
                    u = 1.0
            except:
                u = 0 # suspect PM_ERR_AGAIN from first-step rate-calculation
                
            self.cpu_util += u
            if u > self.peak_cpu_util:
                self.peak_cpu_util = u
                self.peak_cpu = ucode
