package Perl::Dist::WiX::Asset::LaunchExe;
...
sub install {
	...
	my $to =
	  catfile( $self->_get_image_dir(), 'local', 'bin', $bin . '.exe' );
	unless ( -f $to ) {
		PDWiX::File->throw(
			file    => $to,
...
Perl::Dist::WiX::Asset::LaunchExe - Start menu launcher asset for exe files

=head1 SYNOPSIS

  my $distribution = Perl::Dist::WiX::Asset::LaunchExe->new(
    ...
  );
...
and returns a new B<Perl::Dist::WiX::Asset::LaunchExe> object.
...
The C<new> method returns a B<Perl::Dist::WiX::Asset::LaunchExe> object,
