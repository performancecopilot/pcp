...
ParseSocket(char *pmDomainLabel, int pmDomainId)
{
	...
	if (service)
		...
		nLines, socketName, netstrerror(neterror()));
	    free(pmDomainLabel);
	    free(socketName);
	    return -1;
	}
...
ConnectSocketAgent(AgentInfo *aPtr)
{
	...
	if (fd < 0) {
	    fprintf(stderr,
		     ...
		     aPtr->pmDomainLabel, netstrerror(neterror()));
	    return -1;
	}
...
CreateAgentPOSIX(AgentInfo *aPtr)
{
	...
	if (pipe1(inPipe) < 0) {
	    fprintf(stderr,
		    ...
		    aPtr->pmDomainLabel, osstrerror(oserror()));
	    return -1;
	}
...
CreateAgentWin32(AgentInfo *aPtr)
{
	...
	fprintf(stderr, "pmcd: CreateProcess for \"%s\" agent: %s: %s\n",
			aPtr->pmDomainLabel, command, osstrerror(oserror()));
	return -1;
    }
