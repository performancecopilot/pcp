...
<br>&nbsp;&nbsp;&nbsp;[8.2 The _triaged_ file](#the-triaged-file)
<br>&nbsp;&nbsp;&nbsp;[8.3 The _localconfig_ file](#the-localconfig-file)
...
<br>&nbsp;&nbsp;&nbsp;[_common.config_ files](#common.config-files)
...
**simple** PMDAs need to be installed (however the QA infrastructure
...
Historical record of each execution of [**check**]((#check-script),
...
|**$PCPQA\_CLOSE\_X\_SERVER**|See the [_common.config_ files](#common.config-files) section.|
|**$PCPQA\_DESKTOP\_HACK**|See the [_common.config_ files](#common.config-files) section.|
|**$PCPQA\_FAR\_PMCD**|See the [_common.config_ files](#common.config-files) section.|
|**$PCPQA\_HYPHEN\_HOST**</a>|See the [_common.config_ files](#common.config-files) section.|
...
## 8.2 The <a id="idx+files+triaged">_triaged_</a> file
...
## 8.3 The <a id="idx+files+localconfig">_localconfig_</a> file
...
used ones are described in the table below), _common.check_ also
handles:
...
|<a id="idx+funcs+checkkeyserverping">**\_check\_key\_server\_ping**</a>|Usage: **\_check\_key\_server\_ping** _port_<br>Send "ping" requests to the local key server on port _port_ until we see the expected response.  Will timeout after 1.25 secs and output from key server will be printed, else no output or return value.|
...
|<a id="idx+funcs+hosttofqdn">**\_host\_to\_fqdn**</a>|Usage: **\_host\_to\_fqdn** _hostname_<br>Output the FQDN (fully qualified host name) for _hostname_.|
|<a id="idx+funcs+hosttoipaddr">**\_host\_to\_ipaddr**</a>|Usage: **\_host\_to\_ipaddr** _hostname_<br>Output the IPv4 address of _hostname_.|
...
|<a id="idx+funcs+ipaddrtohost">**\_ipaddr\_to\_host**</a>|Usage: **\_ipaddr\_to\_host** _ipaddr_<br>Output the hostname associated with the IPv4 address _ipaddr_.|
...
from _/dev/tty_, else do not re-install the PMDA.
...
|<a id="idx+funcs+filteroptionalpmdainstances">**\_filter\_optional\_pmda\_instances**</a>|Input is **pminfo**(1) **-f** output (or similar) for the `pmcd.agent` metrics. The configuration of PMDAs that have been installed for **pmcd**(1) to communicate varies from platform to platform and time to time. This filter removes the instance lines for optional PMDAs that may, or may not, be active.|
|<a id="idx+funcs+filteroptionalpmdas">**\_filter\_optional\_pmdas**</a>|Input could be a **pmcd**(1) log file or output from **ps**(1). The configuration of PMDAs that have been installed for **pmcd**(1) to communicate varies from platform to platform and time to time. This filter removes lines that contain the name of the PMDA's executable for all optional PMDAs.|
...
|<a id="idx+funcs+filterpmdainstall">**\_filter\_pmda\_install**</a>|Input is from a PMDA's _Install_ script. Remove the non-deterministic chatter from installing a PMDA and notifying **pmcd**(1).<br>Calls [**\_filter\_pcp\_start**](#idx+funcs+filterpcpstart).|
|<a id="idx+funcs+filterpmdaremove">**\_filter\_pmda\_remove**</a>|Input is from a PMDA's _Remove script. Remove the non-deterministic chatter from uninstalling a PMDA and notifying **pmcd**(1).<br>Calls [**\_filter\_pmda\_install**](#idx+funcs+filterpcpstart).|
...
|<a id="idx+funcs+filtertoppmns">**\_filter\_top\_pmns**</a>|The [**PMNS**](#idx+pmns) contains top-level metric names that depend on the configuration of PMDAs that have been installed for **pmcd**(1) to communicate with. This filter removes the lines for top-level PMNS names that are associated with optional PMDAs that may, or may not, be in the reported PMNS.| See the [**mk.logfarm**](#mk.logfarm-script) section. Refer to the [**\_cleanup\_pmda**](#idx+funcs+cleanuppmda) function below. [**\_notrun**](#idx+funcs+notrun) is called with an appropriate message. [**$seq**](#idx+vars+seq) The unfiltered input to **foo** is appended to [**$seq\_full**](#idx+vars+seqfull). 
...
|<a id="idx+funcs+instancesfilterany">**\_instances\_filter\_any**</a>|Input is **pminfo**(1) **-f** output (or similar). If any metric instance has a value, output "OK" else output nothing.|
|<a id="idx+funcs+instancesfilterexact">**\_instances\_filter\_exact**</a>|Usage: **\_instances\_filter\_exact** _match_<br>Input is **pminfo**(1) **-f** output (or similar). If any metric instance has a value equal to _match_, output "OK" else output nothing.|
|<a id="idx+funcs+instancesfilternonzero">**\_instances\_filter\_nonzero**</a>|Input is **pminfo**(1) **-f** output (or similar). If any metric instance has a value and the value is greater than zero, output "OK" else output nothing.|
|<a id="idx+funcs+instvaluefilter">**\_inst\_value\_filter**</a>|Input is **pminfo**(1) **-f** output (or similar). Concatenate lines so that each metric instance has one value line.  Likely use is for `proc` metrics where the values are strings and may contain embedded newlines.|
...
|<a id="idx+funcs+sortpmdumplogd">**\_sort\_pmdumplog\_d**</a>|Input is from **pmlogdump**(1) with the **-d** option. This filter sorts the metadata into ascending [PMID](#idx+pmns) sequence and calls _src/hex2nbo_ to convert any hexadecimal numbers into network byte order.|
|<a id="idx+funcs+valuefilterany">**\_value\_filter\_any**</a>|Input is **pminfo**(1) **-f** output (or similar). For all lines, if a metric instance has a value, replace the value with **OK**.|
|<a id="idx+funcs+valuefilternonzero">**\_value\_filter\_nonzero**</a>|Input is **pminfo**(1) **-f** output (or similar). For all lines, if a metric instance has a value and the value is greater than zero, replace the value with **OK**.|
...
|<a id="idx+cmds+appchange">**appchange**</a>|Usage: **appchange** \[**-c**] _app1_ \[_app2_ ...]<br>Recheck all QA tests that appear to use the test application _src/app1_ or _src/app2_ or ... **${TMPDIR:-/tmp}**_/appcache_ is a cache of mappings between test sequence numbers and uses, for all applications in _src/_ \... **appchange** will build the cache if it is not already there, use **-c** to clear and rebuild the cache.|
...
|<a id="idx+cmds+check-setup">**check-setup**</a>|Check QA environment is as expected. Documented in _README_ but not used otherwise.|
|<a id="idx+cmds+check-vars">**check-vars**</a>|Check shell variables across the _common\*_ "include" files and the scripts used to run and manage QA. For the most part, the _common\*_ files should use a "\_\_" prefix for shell variables\[2] to insulate them from the use of arbitrarily name shell variables in the QA tests themselves (all of which "source" multiple of the _common\*_ files). **check-vars** also includes some exceptions which are a useful cross-reference.|
...
|broken|A family of PMDAs that are broken and exercise error paths and recovery for **libpcp\_pmda** and **pmcd**(1). See comments at the start of _pmdas/broken.c_ for details.|
...
|whacko|Leverages the **trivial** PMDA to exercise integrity checking in the common shell functions used for PMDA _Install_ and _Remove_ scripts.|
...
**valgrind** and not PCP.  This is done in one of several ways:
...
The **-m** (missing) option lists packages in the _packing-list_ file but not currently installed.
The **-i** (include) or **-x** (exclude) options may be used to refine the set of packages checked, based on the _tag_.

If there is no matching _packing-list_ file a new one will be created.
...
If you find something that does not work or seem right, then either
...
privileges are needed, e.g. to stop or start **pmcd**(1), **Install**
or **Remove** PMDAs, etc. the **sudo**(1) application is used. When
...
In addition to the standard **pmcd**(1) port(s) (TCP ports 44321,
...
## <a id="idx+files+common.config">_common.config_</a> files
...
updates, e.g. "xhost +local:" for such tests to pass (e.g. test 325).
...
|<a id="idx+pmapi">**PMAPI**</a>|**P**erformance **M**etrics **A**pplication **I**nterface: the public interfaces supported by _libpcp_|
...
.\" +ok+ br {from <br>} cd'd cd's cgroup {from /proc/pid/cgroup}
...
|---|---|---|---|
|[PCP](#idx+pcp)|[\_check\_job\_scheduler](#idx+funcs+checkjobscheduler)|[\_get\_config](#idx+funcs+getconfig)|[\_try\_pmlc](#idx+funcs+trypmlc)|
|[PMAPI](#idx+pmapi)|[\_check\_key\_server](#idx+funcs+checkkeyserver)|[\_get\_endian](#idx+funcs+getendian)|[\_value\_filter\_any](#idx+funcs+valuefilterany)|
|[PMCD](#idx+pmcd)|[\_check\_key\_server\_ping](#idx+funcs+checkkeyserverping)|[\_get\_fqdn](#idx+funcs+getfqdn)|[\_value\_filter\_nonzero](#idx+funcs+valuefilternonzero)|
|[PMDA](#idx+pmda)|[\_check\_key\_server\_version](#idx+funcs+checkkeyserverversion)|[\_get\_libpcp\_config](#idx+funcs+getlibpcpconfig)|[\_wait\_for\_pmcd](#idx+funcs+waitforpmcd)|
|[PMID](#idx+pmid)|[\_check\_key\_server\_version\_offline](#idx+funcs+checkkeyserverversionoffline)|[\_get\_port](#idx+funcs+getport)|[\_wait\_for\_pmcd\_stop](#idx+funcs+waitforpmcdstop)|
|[PMNS](#idx+pmns)|[\_check\_local\_primary\_archive](#idx+funcs+checklocalprimaryarchive)|[\_get\_primary\_logger\_pid](#idx+funcs+getprimaryloggerpid)|[\_wait\_for\_pmie](#idx+funcs+waitforpmie)|
|**Commands and Scripts**|[\_check\_metric](#idx+funcs+checkmetric)|[\_get\_word\_size](#idx+funcs+getwordsize)|[\_wait\_for\_pmlogger](#idx+funcs+waitforpmlogger)|
|[admin/list-packages](#idx+cmds+list-packages)|[\_check\_search](#idx+funcs+checksearch)|[\_host\_to\_fqdn](#idx+funcs+hosttofqdn)|[\_wait\_for\_pmproxy](#idx+funcs+waitforpmproxy)|
|[all-by-group](#idx+cmds+all-by-group)|[\_check\_series](#idx+funcs+checkseries)|[\_host\_to\_ipaddr](#idx+funcs+hosttoipaddr)|[\_wait\_for\_pmproxy\_logfile](#idx+funcs+waitforpmproxylogfile)|
|[appchange](#idx+cmds+appchange)|[\_check\_valgrind](#idx+funcs+checkvalgrind)|[\_host\_to\_ipv6addrs](#idx+funcs+hosttoipv6addrs)|[\_wait\_for\_pmproxy\_metrics](#idx+funcs+waitforpmproxymetrics)|
|[bad-by-group](#idx+cmds+bad-by-group)|[\_clean\_display](#idx+funcs+cleandisplay)|[\_instances\_filter\_any](#idx+funcs+instancesfilterany)|[\_wait\_for\_port](#idx+funcs+waitforport)|
|[check](#idx+cmds+check)|[\_cleanup\_pmda](#idx+funcs+cleanuppmda)|[\_instances\_filter\_exact](#idx+funcs+instancesfilterexact)|[\_wait\_pmcd\_end](#idx+funcs+waitpmcdend)|
|[check.app.ok](#idx+cmds+check.app.ok)|[\_cull\_dup\_lines](#idx+funcs+cullduplines)|[\_instances\_filter\_nonzero](#idx+funcs+instancesfilternonzero)|[\_wait\_pmie\_end](#idx+funcs+waitpmieend)|
|[check-auto](#idx+cmds+check-auto)|[\_disable\_loggers](#idx+funcs+disableloggers)|[\_inst\_value\_filter](#idx+funcs+instvaluefilter)|[\_wait\_pmlogctl](#idx+funcs+waitpmlogctl)|
|[check-flakey](#idx+cmds+check-flakey)|[\_domain\_name](#idx+funcs+domainname)|[\_ipaddr\_to\_host](#idx+funcs+ipaddrtohost)|[\_wait\_pmlogger\_end](#idx+funcs+waitpmloggerend)|
|[check-group](#idx+cmds+check-group)|[\_exit](#idx+funcs+exit)|[\_ipv6\_localhost](#idx+funcs+ipv6localhost)|[\_wait\_pmproxy\_end](#idx+funcs+waitpmproxyend)|
|[check-pdu-coverage](#idx+cmds+check-pdu-coverage)|[\_fail](#idx+funcs+fail)|[\_libvirt\_is\_ok](#idx+funcs+libvirtisok)|[\_wait\_process\_end](#idx+funcs+waitprocessend)|
|[check-setup](#idx+cmds+check-setup)|[\_filesize](#idx+funcs+filesize)|[\_machine\_id](#idx+funcs+machineid)|[\_webapi\_header\_filter](#idx+funcs+webapiheaderfilter)|
|[check-vars](#idx+cmds+check-vars)|[\_filterall\_pcp\_start](#idx+funcs+filterallpcpstart)|[\_make\_helptext](#idx+funcs+makehelptext)|[\_webapi\_response\_filter](#idx+funcs+webapiresponsefilter)|
|[cull-pmlogger-config](#idx+cmds+cull-pmlogger-config)|[\_filter\_compiler\_babble](#idx+funcs+filtercompilerbabble)|[\_make\_proc\_stat](#idx+funcs+makeprocstat)|[\_within\_tolerance](#idx+funcs+withintolerance)|
|[daily-cleanup](#idx+cmds+daily-cleanup)|[\_filter\_console](#idx+funcs+filterconsole)|[\_need\_metric](#idx+funcs+needmetric)|[\_writable\_primary\_logger](#idx+funcs+writableprimarylogger)|
|[find-app](#idx+cmds+find-app)|[\_filter\_cron\_scripts](#idx+funcs+filtercronscripts)|[\_notrun](#idx+funcs+notrun)|**Shell Variables**|
|[find-bound](#idx+cmds+find-bound)|[\_filter\_dbg](#idx+funcs+filterdbg)|[\_path\_readable](#idx+funcs+pathreadable)|[$DSOSUFFIX](#idx+vars+dsosuffix)|
|[find-metric](#idx+cmds+find-metric)|[\_filter\_dumpresult](#idx+funcs+filterdumpresult)|[\_pid\_in\_container](#idx+funcs+pidincontainer)|[$grind\_extra](#idx+vars+grindextra)|
|[flakey-summary](#idx+cmds+flakey-summary)|[\_filter\_helgrind](#idx+funcs+filterhelgrind)|[\_prefer\_valgrind](#idx+funcs+prefervalgrind)|[$here](#idx+vars+here)|
|[getpmcdhosts](#idx+cmds+getpmcdhosts)|[\_filter\_init\_distro](#idx+funcs+filterinitdistro)|[\_prepare\_pmda](#idx+funcs+preparepmda)|[$PCP\_\*](#idx+vars+pcpstar)|
|[grind](#idx+cmds+grind)|[\_filter\_ls](#idx+funcs+filterls)|[\_prepare\_pmda\_install](#idx+funcs+preparepmdainstall)|[$PCPQA\_CLOSE\_X\_SERVER](#idx+vars+pcpqaclosexserver)|
|[grind-pmda](#idx+cmds+grind-pmda)|[\_filter\_optional\_labels](#idx+funcs+filteroptionallabels)|[\_prepare\_pmda\_mmv](#idx+funcs+preparepmdammv)|[$PCPQA\_DESKTOP\_HACK](#idx+vars+pcpqadesktophack)|
|[group-stats](#idx+cmds+group-stats)|[\_filter\_optional\_pmda\_instances](#idx+funcs+filteroptionalpmdainstances)|[\_private\_pmcd](#idx+funcs+privatepmcd)|[$PCPQA\_FAR\_PMCD](#idx+vars+pcpqafarpmcd)|
|[mk.localconfig](#idx+cmds+mk.localconfig)|[\_filter\_optional\_pmdas](#idx+funcs+filteroptionalpmdas)|[\_ps\_tcp\_port](#idx+funcs+pstcpport)|[$PCPQA\_HYPHEN\_HOST](#idx+vars+pcpqahyphenhost)|
|[mk.logfarm](#idx+cmds+mk.logfarm)|[\_filter\_pcp\_restart](#idx+funcs+filterpcprestart)|[\_pstree\_all](#idx+funcs+pstreeall)|[$PCPQA\_IN\_CI](#idx+vars+pcpqainci)|
|[mk.qa\_hosts](#idx+cmds+mk.qahosts)|[\_filter\_pcp\_start](#idx+funcs+filterpcpstart)|[\_pstree\_oneline](#idx+funcs+pstreeoneline)|[$PCPQA\_PREFER\_VALGRIND](#idx+vars+pcpqaprefervalgrind)|
|[mk.variant](#idx+cmds+mk.variant)|[\_filter\_pcp\_start\_distro](#idx+funcs+filterpcpstartdistro)|[\_quote\_filter](#idx+funcs+quotefilter)|[$PCPQA\_SYSTEMD](#idx+vars+pcpqasystemd)|
|[new](#idx+cmds+new)|[\_filter\_pcp\_stop](#idx+funcs+filterpcpstop)|[\_remove\_job\_scheduler](#idx+funcs+removejobscheduler)|[$pmcd\_args](#idx+vars+pmcdargs)|
|[new-dup](#idx+cmds+new-dup)|[\_filter\_pmcd\_log](#idx+funcs+filterpmcdlog)|[\_restore\_auto\_restart](#idx+funcs+restoreautorestart)|[$pmcd\_pid](#idx+vars+pmcdpid)|
|[new-grind](#idx+cmds+new-grind)|[\_filter\_pmda\_install](#idx+funcs+filterpmdainstall)|[\_restore\_config](#idx+funcs+restoreconfig)|[$pmcd\_port](#idx+vars+pmcdport)|
|[new-seqs](#idx+cmds+new-seqs)|[\_filter\_pmda\_remove](#idx+funcs+filterpmdaremove)|[\_restore\_job\_scheduler](#idx+funcs+restorejobscheduler)|[$seq](#idx+vars+seq)|
|[really-retire](#idx+cmds+really-retire)|[\_filter\_pmdumplog](#idx+funcs+filterpmdumplog)|[\_restore\_loggers](#idx+funcs+restoreloggers)|[$seq\_full](#idx+vars+seqfull)|
|[recheck](#idx+cmds+recheck)|[\_filter\_pmdumptext](#idx+funcs+filterpmdumptext)|[\_restore\_pmda\_install](#idx+funcs+restorepmdainstall)|[$status](#idx+vars+status)|
|[remake](#idx+cmds+remake)|[\_filter\_pmie\_log](#idx+funcs+filterpmielog)|[\_restore\_pmda\_mmv](#idx+funcs+restorepmdammv)|[$sudo](#idx+vars+sudo)|
|[sameas](#idx+cmds+sameas)|[\_filter\_pmie\_start](#idx+funcs+filterpmiestart)|[\_restore\_pmlogger\_control](#idx+funcs+restorepmloggercontrol)|[$tmp](#idx+vars+tmp)|
|[show-me](#idx+cmds+show-me)|[\_filter\_pmie\_stop](#idx+funcs+filterpmiestop)|[\_restore\_primary\_logger](#idx+funcs+restoreprimarylogger)|[$valgrind\_clean\_assert](#idx+vars+valgrindcleanassert)|
|[var-use](#idx+cmds+var-use)|[\_filter\_pmproxy\_log](#idx+funcs+filterpmproxylog)|[\_run\_helgrind](#idx+funcs+runhelgrind)|**Files**|
|[whatami](#idx+cmds+whatami)|[\_filter\_pmproxy\_start](#idx+funcs+filterpmproxystart)|[\_run\_valgrind](#idx+funcs+runvalgrind)|[$seq\_full](#idx+files+seqfull)|
|**Shell Functions**|[\_filter\_pmproxy\_stop](#idx+funcs+filterpmproxystop)|[\_save\_config](#idx+funcs+saveconfig)|[check.log](#idx+files+check.log)|
|[\_all\_hostnames](#idx+funcs+allhostnames)|[\_filter\_post](#idx+funcs+filterpost)|[\_service](#idx+funcs+service)|[check.time](#idx+files+check.time)|
|[\_all\_ipaddrs](#idx+funcs+allipaddrs)|[\_filter\_slow\_pmie](#idx+funcs+filterslowpmie)|[\_set\_dsosuffix](#idx+funcs+setdsosuffix)|[_common.\*_](#idx+files+common.star)|
|[\_arch\_start](#idx+funcs+archstart)|[\_filter\_top\_pmns](#idx+funcs+filtertoppmns)|[\_show\_pmie\_errors](#idx+funcs+showpmieerrors)|[_common_](#idx+files+common)|
|[\_avail\_metric](#idx+funcs+availmetric)|[\_filter\_torture\_api](#idx+funcs+filtertortureapi)|[\_show\_pmie\_exit](#idx+funcs+showpmieexit)|[_common.config_](#idx+files+common.config)|
|[\_change\_config](#idx+funcs+changeconfig)|[\_filter\_valgrind](#idx+funcs+filtervalgrind)|[\_sighup\_pmcd](#idx+funcs+sighuppmcd)|[group](#idx+files+group)|
|[\_check\_64bit\_platform](#idx+funcs+check64bitplatform)|[\_filter\_valgrind\_possibly](#idx+funcs+filtervalgrindpossibly)|[\_sort\_pmdumplog\_d](#idx+funcs+sortpmdumplogd)|[_localconfig_](#idx+files+localconfig)|
|[\_check\_agent](#idx+funcs+checkagent)|[\_filter\_views](#idx+funcs+filterviews)|[\_start\_up\_pmlogger](#idx+funcs+startuppmlogger)|[qa\_hosts](#idx+files+qahosts)|
|[\_check\_core](#idx+funcs+checkcore)|[\_find\_free\_port](#idx+funcs+findfreeport)|[\_stop\_auto\_restart](#idx+funcs+stopautorestart)|[qa\_hosts.primary](#idx+files+qahosts.primary)|
|[\_check\_display](#idx+funcs+checkdisplay)|[\_find\_key\_server\_modules](#idx+funcs+findkeyservermodules)|[\_systemctl\_status](#idx+funcs+systemctlstatus)|[_triaged_](#idx+files+triaged)|
|[\_check\_freespace](#idx+funcs+checkfreespace)|[\_find\_key\_server\_name](#idx+funcs+findkeyservername)|[\_triage\_pmcd](#idx+funcs+triagepmcd)||
|[\_check\_helgrind](#idx+funcs+checkhelgrind)|[\_find\_key\_server\_search](#idx+funcs+findkeyserversearch)|[\_triage\_wait\_point](#idx+funcs+triagewaitpoint)||
